...
import type { ITimetableEntry, ITimetableTask, ShiftType, TaskCategory, TaskStatus } from "@/interfaces/timetable.interface";

const createRandomTask = (inspector: typeof MOCK_INSPECTORS[0], date: string, startHour: number, durationMinutes: number): ITimetableTask => {
    const startTime = `${String(startHour).padStart(2, '0')}:00`;
    const endHour = startHour + Math.floor(durationMinutes / 60);
    const endMinute = durationMinutes % 60;
    const endTime = `${String(endHour).padStart(2, '0')}:${String(endMinute).padStart(2, '0')}`;

    const categories: TaskCategory[] = ['BRIEFING', 'MEETING', 'MONITORING', 'CONSULTATION', 'INFRASTRUCTURE', 'SOCIAL_AID', 'INSPECTION', 'OTHER'];
...
const createDailyTimetable = (inspector: typeof MOCK_INSPECTORS[0], date: string, shiftType: ShiftType): ITimetableEntry => {
...
for (let i = 0; i < 7; i++) {
    ...
    MOCK_INSPECTORS.forEach(inspector => {
        MOCK_TIMETABLES.push(createDailyTimetable(inspector, dateString, 'MORNING'));
        MOCK_TIMETABLES.push(createDailyTimetable(inspector, dateString, 'AFTERNOON'));
    });
}
